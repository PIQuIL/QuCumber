...
from data import extract_refbasis_samples, load_data
...
def train_real(tr_samples_path, target_psi_path, num_hidden, epochs, batch_size,
    ...
    if (num_visible < 20):
        ...
        callbacks = [MetricEvaluator(
            10, {'Fidelity': ts.fidelity, 'KL': ts.KL}, target_psi=target_psi)]
...
def train_complex(tr_samples_path, tr_bases_path, target_psi_path, bases_path, num_hidden, epochs, batch_size, num_chains, k, learning_rate,
    ...
    train_samples, target_psi, train_bases, bases = load_data(
        tr_samples_path, target_psi_path, tr_bases_path, bases_path)
    num_bases = len(bases)
    ...
    if (num_visible < 20):
        nn_state.space = nn_state.generate_Hilbert_space(num_visible)
        callbacks = [MetricEvaluator(
            1, {'Fidelity': ts.fidelity, 'KL': ts.KL}, target_psi=target_psi, bases=bases)]
