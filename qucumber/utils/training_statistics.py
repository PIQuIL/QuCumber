    ...
    for x in range(1 << N):
        ...
        for xp in range(1 << num_nontrivial_U):
            ...
            for ii in range(num_nontrivial_U):
                tmp = unitaries[basis[nontrivial_sites[ii]]][:, int(
                    nn_state.space[x][nontrivial_sites[ii]]), int(v[nontrivial_sites[ii]])].to(nn_state.device)
...
def KL(nn_state, target_psi, bases=None):
    psi_r = torch.zeros(2, 1 << nn_state.num_visible,
    ...
    if bases is None:
        ...
        for b in range(1, len(bases)):
            ...
            target_psi_r = rotate_psi(
                nn_state, bases[b], unitary_dict, target_psi)
