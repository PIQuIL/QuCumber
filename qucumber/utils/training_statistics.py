...
def fidelity(nn_state,target_psi,bases=None,target_psi_dict=None):
...
def KL(nn_state,target_psi,bases=None,target_psi_dict=None):
    ...
    if bases is None:
        ...
        if target_psi_dict is None:
            raise ValueError('Missing the rotated target wavefunctions')
        else:
            num_bases = len(bases)
            for b in range(1,len(bases)):
                psi_r = rotate_psi(nn_state,bases[b],unitary_dict)
                for ii in range(len(space)):
                    if(cplx.norm(target_psi_dict[bases[b]][:,ii])>0.0):
                        KL += cplx.norm(target_psi_dict[bases[b]][:,ii])*cplx.norm(target_psi_dict[bases[b]][:,ii]).log()
                    KL -= cplx.norm(target_psi_dict[bases[b]][:,ii])*cplx.norm(psi_r[:,ii]).log().item()
                    KL += cplx.norm(target_psi_dict[bases[b]][:,ii])*nn_state.Z.log()
    return  KL/float(num_bases)    

def load_psi_dict(psi_data,bases_data):
    ...
    return bases,target_psi,target_psi_dict
...
#        vis = self.generate_visible_space(self.N) 
...
#            psi_r = self.rotate_psi(nn_state,self.bases[b])
#            for ii in range(len(self.vis)):
#                if(cplx.norm(self.target_psi_dict[self.bases[b]][:,ii])>0.0):
#                    KL += cplx.norm(self.target_psi_dict[self.bases[b]][:,ii])*cplx.norm(self.target_psi_dict[bases[b]][:,ii]).log()/float(len(self.bases))
#                KL -= cplx.norm(self.target_psi_dict[self.bases[b]][:,ii])*cplx.norm(psi_r[:,ii]).log().item()/float(len(self.bases))
#                KL += cplx.norm(self.target_psi_dict[self.bases[b]][:,ii])*self.Z.log()/float(len(self.bases))
#        self.KL = KL    
#
