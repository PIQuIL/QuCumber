...
def grad_positive(k,num_chains,seed,eps):
...
def grad_complex(k,num_chains,seed,eps):
    ...
    target_psi=torch.tensor(test_data['2qubits']['target_psi'],dtype = torch.double)
    nh = train_samples.shape[-1]
    bases = test_complex.transform_bases(bases_data)
    unitary_dict = test_complex.unitaries.create_dict()
    target_psi_dict = test_complex.load_target_psi(bases,target_psi) 
    vis = test_positive.generate_visible_space(train_samples.shape[-1]) 
    nn_state = ComplexWavefunction(num_visible=train_samples.shape[-1],
                                   num_hidden=nh)
    qr = QuantumReconstruction(nn_state)
    eps= 1.e-6
    test_complex.run(qr,target_psi_dict,train_samples,train_bases,unitary_dict,bases,vis,eps,k) 
