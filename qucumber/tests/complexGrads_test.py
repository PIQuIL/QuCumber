...
from utils import cplx, unitaries
...
def rotate_psi(nn_state, basis, unitary_dict, vis):
    ...
    for x in range(1 << N):
        ...
        for xp in range(1 << num_nontrivial_U):
            ...
            U = torch.tensor([1., 0.], dtype=torch.double)
            for ii in range(num_nontrivial_U):
                tmp = unitary_dict[basis[nontrivial_sites[ii]]][:, int(
                    vis[x][nontrivial_sites[ii]]), int(v[nontrivial_sites[ii]])]
                U = cplx.scalar_mult(U, tmp)
...
def compute_numerical_NLL(nn_state, data_samples, data_bases, Z, unitary_dict, vis):
    ...
    for i in range(batch_size):
        ...
        if (b_flag == 0):
            NLL -= (probability(nn_state,
                                data_samples[i], Z)).log().item()/batch_size
        else:
...
def numeric_gradNLL(nn_state, data_samples, data_bases, unitary_dict, param, vis, eps):
    ...
    for i in range(len(param)):
        ...
        NLL_p = compute_numerical_NLL(
            nn_state, data_samples, data_bases, Z, unitary_dict, vis)

        param[i] -= 2*eps

        Z = partition(nn_state, vis)
        NLL_m = compute_numerical_NLL(
            nn_state, data_samples, data_bases, Z, unitary_dict, vis)
...
def numeric_gradKL(param, nn_state, psi_dict, vis, unitary_dict, bases, eps):
    ...
    for i in range(len(param)):
        ...
        Z = partition(nn_state, vis)
        KL_p = compute_numerical_kl(
            nn_state, psi_dict, vis, Z, unitary_dict, bases)

        param[i] -= 2*eps

        Z = partition(nn_state, vis)
        KL_m = compute_numerical_kl(
            nn_state, psi_dict, vis, Z, unitary_dict, bases)
...
def algorithmic_gradKL(nn_state, psi_dict, vis, unitary_dict, bases):
    ...
    grad_KL = [torch.zeros(nn_state.rbm_am.num_pars, dtype=torch.double), torch.zeros(
        nn_state.rbm_ph.num_pars, dtype=torch.double)]
    Z = partition(nn_state, vis)

    for i in range(len(vis)):
        grad_KL[0] += cplx.norm(psi_dict[bases[0]][:, i]) * \
            nn_state.rbm_am.effective_energy_gradient(vis[i])/float(len(bases))
...
def run(qr, psi_dict, data_samples, data_bases, unitary_dict, bases, vis, eps, k):
    ...
    alg_grad_KL = algorithmic_gradKL(
        qr.nn_state, psi_dict, vis, unitary_dict, bases)
    for n, net in enumerate(qr.nn_state.networks):
        counter = 0
        print('\n\nRBM: %s' % net)
        rbm = getattr(qr.nn_state, net)

        num_grad_KL = numeric_gradKL(
            rbm.weights.view(-1), qr.nn_state, psi_dict, vis, unitary_dict, bases, eps)
        num_grad_NLL = numeric_gradNLL(
            qr.nn_state, data_samples, data_bases, unitary_dict, rbm.weights.view(-1), vis, eps)

        print("\nTesting weights...")
        print("Numerical KL\tAlg KL\t\t\tNumerical NLL\tAlg NLL")
        for i in range(len(rbm.weights.view(-1))):
            print("{: 10.8f}\t{: 10.8f}\t\t".format(
                num_grad_KL[i], alg_grad_KL[n][counter].item()), end="", flush=True)
            print("{: 10.8f}\t{: 10.8f}\t\t".format(
                num_grad_NLL[i], alg_grad_NLL[n][i].item()))
            counter += 1

        num_grad_KL = numeric_gradKL(
            rbm.visible_bias, qr.nn_state, psi_dict, vis, unitary_dict, bases, eps)
        num_grad_NLL = numeric_gradNLL(
            qr.nn_state, data_samples, data_bases, unitary_dict, rbm.visible_bias, vis, eps)
        ...
        num_grad_KL = numeric_gradKL(
            rbm.hidden_bias, qr.nn_state, psi_dict, vis, unitary_dict, bases, eps)
        num_grad_NLL = numeric_gradNLL(
            qr.nn_state, data_samples, data_bases, unitary_dict, rbm.hidden_bias, vis, eps)
        print("\nTesting visible bias...")
        print("Numerical KL\tAlg KL\t\t\tNumerical NLL\tAlg NLL")
...
if __name__ == '__main__':
    ...
    eps = 1.e-6
    run(qr, psi_dict, train_samples, train_bases,
        unitary_dict, bases, vis, eps, k)
