...
class ComplexWavefunction(object):
    ...
    def gradient(self, basis, sample):
        ...
        if (num_U == 0):
            ...
            rotated_grad = [torch.zeros(2, self.rbm_am.num_pars, dtype=torch.double, device=self.device), torch.zeros(
                2, self.rbm_ph.num_pars, dtype=torch.double, device=self.device)]
            ...
            for x in range(1 << num_U):
                ...
                for ii in range(num_U):
                    tmp = self.unitary_dict[basis[rotated_sites[ii]]][:, int(
                        sample[rotated_sites[ii]]), int(vp[rotated_sites[ii]])]
                    U = cplx.scalar_mult(U, tmp.to(self.device))

                # Gradient on the current configuration
                grad_vp = [self.rbm_am.effective_energy_gradient(
                    vp), self.rbm_ph.effective_energy_gradient(vp)]
    ...
    def sample(self, k):
        ...
        """
        for _ in range(k):
            self.hidden_state = self.rbm_am.sample_h_given_v(
                self.visible_state)
            self.visible_state = self.rbm_am.sample_v_given_h(
                self.hidden_state)
    ...
    def save(self, location, metadata={}):
        ...
        data = {"rbm_am": self.rbm_am.state_dict(
        ), "rbm_ph": self.rbm_ph.state_dict(), **metadata}
