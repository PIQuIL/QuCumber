...
class TestAllGrads(unittest.TestCase):
    ...
    def assertPercentDiff(self, a, b, pdiff, msg=None):
        result = torch.ge(
            pdiff * torch.ones_like(self.percent_diff(a, b)), self.percent_diff(a, b)
        )
    ...
    def test_posgrads(self):
        ...
        num_grad_nll = PGU.numeric_gradNLL(
            nn_state.rbm_am.weights.view(-1), data, vis, eps
        )
    ...
    def test_complexgrads(self):
        ...
        nn_state = ComplexWavefunction(unitary_dict, num_visible, num_hidden, gpu=False)
