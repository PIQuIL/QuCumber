...
class TestAllGrads(unittest.TestCase):
    ...
    def assertPercentDiff(self, a, b, pdiff, msg=None):
        result = torch.ge(pdiff * torch.ones_like(self.percent_diff(a, b)),
                          self.percent_diff(a, b))
    ...
    def test_complexgrads(self):
        ...
        unitary_dict = {b: v.to(device=device)
                        for b, v in unitary_dict.items()}
        psi_dict = {b: v.to(device=device) for b, v in psi_dict.items()}

        alg_grad_nll = CGU.algorithmic_gradNLL(qr, train_samples,
                                               train_bases, k)
        alg_grad_kl = CGU.algorithmic_gradKL(psi_dict, vis,
                                             unitary_dict, bases)
        ...
        for n, net in enumerate(qr.nn_state.networks):
            ...
            for i in range(len(rbm.visible_bias)):
                ...
                print("{: 10.8f}\t{: 10.8f}\t\t"
                      .format(num_grad_nll[i],
                              alg_grad_nll[n][counter].item()))
                counter += 1
