export CUDA_VISIBLE_DEVICES=0
output_file='pt_benchmark_gpu.csv'

echo "L,epochs,batch_size,k,time,memory" >> ${output_file}

for size in 4 8 12 16 20 24 28 32 36 40 44 48 52 56 60 64; do
    for epoch in 250; do
        for batch in 32; do
            for k in 1 10; do

                echo "Iterating..."

                echo -n "$size," >> ${output_file}
                echo -n "$epoch," >> ${output_file}
                echo -n "$batch," >> ${output_file}
                echo -n "$k," >> ${output_file}

                /usr/bin/time -f "%e,%M" -o ${output_file} -a  \
                python3 ./run_rbm.py train-alt -L $size -b $batch -e $epoch -k $k -lr 0.001 -m 0.9 \
                > /dev/null 2>&1

            done
        done
    done
done
