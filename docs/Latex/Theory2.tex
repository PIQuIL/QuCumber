...
{\bf
\begin{itemize}
	\item Better monospace font
	\item Better python highlighting (pink!?!)
	\item Intro polishing
	\item Conclusion
	\item Abstract!
\end{itemize}
	\red{
...
\end{python}
		We show in Figure~\ref{fig:KL} the fidelity and KL divergence obtained from the  callbacks. The convergence time will in general depend on the choice of hyperparameters and the complexity of the wavefunction.
...
\end{python}
	...
	\subsubsection{Off-diagonal observables}
		...
		QuCumber provides an interface to sample off-diagonal observables in the \verb|Observable| class. Thorough examples can be found in the tutorial section in the \href{https://qucumber.readthedocs.io/en/stable/}{documentation}. As a simple example, $\sigma^z$ can be easily written as an observable class with
\begin{python}
...
\end{python}
		...
		\begin{figure}[hbt]
			...
			\caption{\label{tfim_magn} Reconstruction of the magnetic observables for the TFIM chain with $N=10$ spins. The results from  We show the average longitudinal (left) and transverse (right) magnetization per site. The dashed line denotes exact results obtained with ED.}
		\end{figure}
