...
RBM are generative models.  Once properly trained, QuCumber can produce a new set of measurement samples.  These samples,  produced in the reference basis, can be used to verify the training of the RBM against the original data set.  More importantly, they can be used to calculate expectation values of observables, both diagonal and off-diagonal in the reference basis.  In fact,
...
setup for which the original data was obtained.  This is particularly relevant for many experiments, where it is easy to imagine many possible observables that are inaccessible, due to fundamental or technical challenges.
