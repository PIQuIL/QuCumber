...
\section*{Abstract}
{\bf
        As we enter the era of intermediate-scale noisy quantum hardware it is increasingly important to confirm the accurate preparation of quantum states on these near-term devices.
        The large number of qubits present in such devices poses the problem of high dimensionality where conventional methods of quantum state tomography break down. 
        In this paper, we present an open-source software package called QuCumber that uses machine learning to effectively reconstruct the most likely quantum state consistent with a set of projective measurements. 
        QuCumber is able to reconstruct wavefunctions for a large number of qubits by representing the wavefunction as a restricted Boltzmann machine.
        This package can be used to study traditionally challenging physical properties, not directly accessible in current experimentse such as the entanglement entropy. 
}
